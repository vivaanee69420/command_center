...
function generateMetrics(slug) {
  ...
  const base = (h % 80) + 60; // 60–140 range factor
...
const PIPELINE_STAGES = [
  { label: "New Lead", count: null, color: "bg-blue-50 text-blue-700 border-blue-200" },
  { label: "Contacted", count: null, color: "bg-amber-50 text-amber-700 border-amber-200" },
  { label: "Consultation Booked", count: null, color: "bg-purple-50 text-purple-700 border-purple-200" },
  { label: "Treatment Accepted", count: null, color: "bg-green-50 text-green-700 border-green-200" },
];
...
function SectionTitle({ icon: Icon, title, iconColor = "text-ink" }) {
  return (
...
function StatBadge({ label, value, bgColor = "bg-green-50", textColor = "text-green-700", size = "md" }) {
  return (
    <div className={cn("rounded-lg px-3 py-2 flex flex-col items-center", bgColor, size === "sm" ? "min-w-[80px]" : "min-w-[100px]")}>
      <span className={cn("font-bold", textColor, size === "sm" ? "text-lg" : "text-2xl")}>{value}</span>
      <span className={cn("text-[10px] font-semibold uppercase tracking-wide mt-0.5", textColor, "opacity-80")}>{label}</span>
    </div>
  );
}

// ─── Tab: Dashboard ─────────────────────────────────────────────────────────
function TabDashboard({ slug, metrics, biz }) {
  const progressPct = Math.min(100, (metrics.revenue / metrics.target) * 100);
  const stageDistrib = [
    Math.round(metrics.pipelineOpen * 0.4),
    Math.round(metrics.pipelineOpen * 0.3),
    Math.round(metrics.pipelineOpen * 0.2),
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#10b981" }}>
          <div className="flex items-start justify-between mb-2">
            <span className="text-[11px] font-semibold text-muted uppercase tracking-wide">Monthly Revenue</span>
        ...
        <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#3b82f6" }}>
          <div className="flex items-start justify-between mb-2">
            <span className="text-[11px] font-semibold text-muted uppercase tracking-wide">Leads This Week</span>
            <span className="flex items-center gap-0.5 text-[11px] font-semibold text-green-600">
              <ArrowUpRight size={13} /> +12%
            </span>
          </div>
          <div className="text-2xl font-bold text-ink">{formatNumber(metrics.leads)}</div>
          <div className="text-xs text-muted mt-1">{metrics.bookings} booked this week</div>
        </Card>

        {/* Bookings */}
        <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#8b5cf6" }}>
          <div className="flex items-start justify-between mb-2">
            <span className="text-[11px] font-semibold text-muted uppercase tracking-wide">Bookings This Week</span>
            <span className="text-[11px] font-semibold text-purple-600">{metrics.convRate}% conv.</span>
          </div>
          <div className="text-2xl font-bold text-ink">{formatNumber(metrics.bookings)}</div>
          <div className="text-xs text-muted mt-1">From {metrics.leads} total leads</div>
        </Card>

        {/* Ad ROI */}
        <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#f59e0b" }}>
          <div className="flex items-start justify-between mb-2">
            <span className="text-[11px] font-semibold text-muted uppercase tracking-wide">Ad ROI</span>
            <Target size={15} className="text-amber-500" />
          </div>
          <div className="text-2xl font-bold text-ink">{metrics.roi}x</div>
          <div className="text-xs text-muted mt-1">Target: 8x+</div>
        </Card>
      </div>

      {/* Revenue Progress */}
      <Card className="p-5">
        <SectionTitle icon={Activity} title="Revenue vs Target" iconColor="text-green-600" />
        <div className="flex items-center gap-4 mb-2">
          <div className="flex-1">
            <div className="h-4 bg-green-50 rounded-full overflow-hidden border border-green-100">
              <div
          ...
          <span className="text-sm font-bold text-ink whitespace-nowrap">{progressPct.toFixed(0)}%</span>
        </div>
        <div className="flex justify-between text-xs text-muted">
          <span>Current: <span className="font-semibold text-ink">{formatCurrency(metrics.revenue)}</span></span>
          <span>Target: <span className="font-semibold text-ink">{formatCurrency(metrics.target)}</span></span>
        </div>
      </Card>

      {/* Sales Pipeline */}
      <Card className="p-5">
        <SectionTitle icon={Layers} title="Sales Pipeline" iconColor="text-blue-600" />
        <div className="flex items-stretch gap-2">
          {PIPELINE_STAGES.slice(0, 3).map((stage, i) => (
            <div key={stage.label} className="flex items-center gap-2 flex-1">
              <div className={cn("flex-1 rounded-lg border p-3 text-center", stage.color)}>
                <div className="text-lg font-bold">{stageDistrib[i] || 0}</div>
                <div className="text-[10px] font-semibold uppercase tracking-wide mt-0.5">{stage.label}</div>
              </div>
              {i < 2 && <ChevronRight size={16} className="text-muted shrink-0" />}
            </div>
          ))}
        </div>
      </Card>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card className="p-4 flex flex-col items-center text-center gap-1">
          ...
          <div className="text-xl font-bold text-ink">{metrics.rating}</div>
          <div className="text-xs text-muted">{formatNumber(metrics.reviews)} reviews</div>
        </Card>
        <Card className="p-4 flex flex-col items-center text-center gap-1">
          <Wifi size={20} className="text-green-500" />
          <div className="text-sm font-bold text-ink">Voice AI</div>
          <span className="text-[10px] font-bold bg-green-100 text-green-700 rounded-full px-2 py-0.5">24/7 ACTIVE</span>
        </Card>
        <Card className="p-4 flex flex-col items-center text-center gap-1">
          <Megaphone size={20} className="text-blue-500" />
          <div className="text-xl font-bold text-ink">{formatCurrency(metrics.adSpend)}</div>
          <div className="text-xs text-muted">Monthly Ad Spend</div>
        </Card>
        <Card className="p-4 flex flex-col items-center text-center gap-1">
          <CalendarCheck size={20} className="text-purple-500" />
          <div className="text-xl font-bold text-ink">{metrics.bookingsToday}</div>
          <div className="text-xs text-muted">Bookings Today</div>
        </Card>
      </div>
    </div>
  );
        ...
        <Card className="p-5" style={{ borderLeftWidth: 4, borderLeftColor: "#10b981" }}>
          ...
          <div className="space-y-2.5">
            <div className="flex justify-between items-center">
              <span className="text-xs text-ink">SEO</span>
              <span className="text-[10px] font-bold bg-green-100 text-green-700 rounded-full px-2 py-0.5">Active</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs text-ink">Social Media</span>
              <span className="text-[10px] font-bold bg-green-100 text-green-700 rounded-full px-2 py-0.5">Active</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs text-ink">Content</span>
              <span className="text-[10px] font-bold bg-amber-100 text-amber-700 rounded-full px-2 py-0.5">3 Pending</span>
            </div>
          </div>
        </Card>
...
function TabSales({ metrics }) {
  const h = slugHash("sales");
  const stages = [
    { label: "New Lead", count: Math.round(metrics.pipelineOpen * 0.35), color: "bg-blue-100 text-blue-800" },
    { label: "Contacted", count: Math.round(metrics.pipelineOpen * 0.28), color: "bg-amber-100 text-amber-800" },
    { label: "Consultation Booked", count: Math.round(metrics.pipelineOpen * 0.22), color: "bg-purple-100 text-purple-800" },
    { label: "Treatment Accepted", count: Math.round(metrics.pipelineOpen * 0.15), color: "bg-green-100 text-green-800" },
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {stages.map((s, i) => (
            <div key={s.label} className="relative">
              <div className={cn("rounded-lg p-3 text-center border", s.color.replace("text-", "border-").replace("100", "200"), s.color)}>
      ...
      {/* Sales KPIs */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card className="p-4 text-center" style={{ borderTopWidth: 3, borderTopColor: "#10b981" }}>
          <div className="text-xl font-bold text-ink">{metrics.convRate}%</div>
          <div className="text-[11px] text-muted mt-1">Booking Rate</div>
        </Card>
        <Card className="p-4 text-center" style={{ borderTopWidth: 3, borderTopColor: "#3b82f6" }}>
          <div className="text-xl font-bold text-ink">{metrics.won}</div>
          <div className="text-[11px] text-muted mt-1">Deals Won</div>
        </Card>
        <Card className="p-4 text-center" style={{ borderTopWidth: 3, borderTopColor: "#8b5cf6" }}>
          <div className="text-xl font-bold text-ink">{metrics.pipelineOpen}</div>
          <div className="text-[11px] text-muted mt-1">Open Deals</div>
        </Card>
        <Card className="p-4 text-center" style={{ borderTopWidth: 3, borderTopColor: "#f59e0b" }}>
          <div className="text-xl font-bold text-ink">{formatCurrency(avgOpenValue)}</div>
          <div className="text-[11px] text-muted mt-1">Avg Open Value</div>
        </Card>
      </div>
...
// ─── Tab: Operations ─────────────────────────────────────────────────────────
function TabOperations({ metrics }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#10b981" }}>
          <div className="flex items-start justify-between mb-2">
            <span className="text-[11px] font-semibold text-muted uppercase tracking-wide">Appointments Today</span>
            <CalendarCheck size={15} className="text-green-500" />
          </div>
          <div className="text-2xl font-bold text-ink">{metrics.appointmentsToday}</div>
          <div className="text-xs text-muted mt-1">Next: 09:00 AM</div>
        </Card>
        <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#3b82f6" }}>
          <div className="flex items-start justify-between mb-2">
            <span className="text-[11px] font-semibold text-muted uppercase tracking-wide">Open Opportunities</span>
            <Target size={15} className="text-blue-500" />
          </div>
          <div className="text-2xl font-bold text-ink">{metrics.pipelineOpen}</div>
          <div className="text-xs text-muted mt-1">{formatCurrency(metrics.pipelineValue)} pipeline value</div>
        </Card>
        <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#8b5cf6" }}>
          <div className="flex items-start justify-between mb-2">
            <span className="text-[11px] font-semibold text-muted uppercase tracking-wide">Team Tasks</span>
            <CheckCircle size={15} className="text-purple-500" />
          </div>
          <div className="text-2xl font-bold text-ink">{metrics.teamTasks}</div>
          <div className="text-xs text-muted mt-1">Open today</div>
        </Card>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {quickLinks.map((lnk) => (
            <button
              ...
              className={cn(
                "flex flex-col items-center gap-2 rounded-xl p-4 border border-line hover:shadow-md transition cursor-pointer",
                lnk.bg
              )}
            >
  ...
  const cards = [
    {
      ...
      sub: `Target: ${formatCurrency(metrics.target)}`,
      bg: "bg-green-50",
      border: "#10b981",
    ...
    },
    {
      ...
      sub: `Google: ${formatCurrency(metrics.googleAds)} · FB: ${formatCurrency(metrics.facebookAds)}`,
      bg: "bg-red-50",
      border: "#ef4444",
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
      {cards.map((c) => (
        <Card
          key={c.label}
          className={cn("p-6", c.bg)}
          style={{ borderLeftWidth: 4, borderLeftColor: c.border }}
        >
      ...
      <div className="grid grid-cols-2 gap-4">
        {actions.map((a) => (
          <button
            ...
            onClick={a.onClick}
            className={cn(
              "flex flex-col items-center gap-3 p-6 rounded-xl text-white font-semibold text-sm transition shadow-sm",
              a.bg
            )}
          >
            <a.icon size={28} />
      ...
      <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#10b981" }}>
        ...
        <ul className="space-y-2">
          {SDR_SCRIPTS.slice(0, 3).map((s) => (
            <li
              key={s.name}
              className="flex items-center justify-between px-3 py-2.5 rounded-lg border border-line hover:bg-green-50 transition cursor-pointer group"
            >
      ...
      <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#3b82f6" }}>
        ...
        <ul className="space-y-2">
          {OBJECTION_HANDLERS.map((o) => (
            <li
              key={o.text}
              className="flex items-center justify-between px-3 py-2.5 rounded-lg border border-line hover:bg-blue-50 transition cursor-pointer"
            >
      ...
      <Card className="p-5" style={{ borderTopWidth: 4, borderTopColor: "#f59e0b" }}>
        ...
        <ul className="space-y-2">
          {VOICE_SCRIPTS.map((v) => (
            <li
              key={v.name}
              className="flex items-center justify-between px-3 py-2.5 rounded-lg border border-line hover:bg-amber-50 transition cursor-pointer group"
            >
...
export default function PracticeDashboardPage() {
  ...
  const biz = BUSINESS_INFO[slug] || {
    name: slug ? slug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) : "Practice",
    emoji: "🏢",
    type: "practice",
    address: "—",
    phone: "—",
      ...
      <main className="p-6 max-w-[1400px] mx-auto w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
            <div
              className="w-14 h-14 rounded-xl flex items-center justify-center text-2xl shadow-sm border border-line"
        ...
        {/* ─── Tab Content ────────────────────────────────────────── */}
        {activeTab === "Dashboard" && <TabDashboard slug={slug} metrics={metrics} biz={biz} />}
        {activeTab === "Marketing" && <TabMarketing metrics={metrics} />}
        {activeTab === "Sales" && <TabSales metrics={metrics} />}
        {activeTab === "Operations" && <TabOperations metrics={metrics} />}
